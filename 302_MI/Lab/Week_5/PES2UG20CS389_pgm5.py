...
class Tensor:
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients is None:
            ...
            if op2.requires_grad:
                operand2.grad += np.multiply(np.matmul(np.ones_like(operand2.arr), operand1.arr).transpose(), gradients)
    ...
    def backward(self, gradients=None):
        ...
        if self.history[0] == 'add':
            g = self.grad_add(gradients)
            if self.history[1]:
                self.history[1].backward(g[0])
            if self.history[2]:
                self.history[2].backward(g[1])

        elif self.history[0] == 'matmul':
            g = self.grad_matmul(gradients)
            if self.history[1]:
                self.history[1].backward(g[0])
            if self.history[2]:
                self.history[2].backward(g[1])
