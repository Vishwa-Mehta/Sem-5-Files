...
class KNN:
    ...
    def find_distance(self, x):
        ...
        for data_pt in x:
            ...
            for point in self.data:
                diff = abs(data_pt - point)
                diff = diff ** self.p
                min_k_dist = 0
                for value in diff:
                    min_k_dist = min_k_dist + value
                min_k_dist = min_k_dist ** (1 / self.p)
                points.append(min_k_dist)
    ...
    def k_neighbours(self, x):
        ...
        """
        # TODO
        point_dist = self.find_distance(x)
        neigh_dist= []
        neigh_index = []

        for point in point_dist:
            lst = []
            index = 0
            for i in point:
                lst.append([i, index])
    ...
    def predict(self, x):
        ...
        """
        # TODO
        neigh_dist,neigh_index = self.k_neighbours(x)
        prediction = []
        eps = 1e-9
        for i in range(0, len(neigh_index)):
            d = dict()
            index = neigh_index[i]
            for j in range(0, len(index)):
                if(self.weighted):
                    if(self.target[index[j]] not in d):
                        d[self.target[index[j]]] = 1/(neigh_dist[i][j] + eps)
                    else:
                        d[self.target[index[j]]]+= 1/(neigh_dist[i][j] + eps)
                else:
                    if(self.target[index[j]] not in d):
                        d[self.target[index[j]]] = 1
                    else:
                        d[self.target[index[j]]] = d[self.target[index[j]]] + 1
            max_value = max(d, key = lambda x: d[x])
            prediction.append(max_value)
        return prediction
        pass
    ...
    def evaluate(self, x, y):
        ...
        """
        # TODO
        user_prediction = self.predict(x)
        count = 0
        for i in range(0, len(y)):
            if(user_prediction[i] == y[i]):
                count = count + 1
